...
export function PostCard({ post, cover, onOpen }: PostCardProps) {
  ...
  const imageSrc = cover.imageUrl ?? "/fake-news.svg";
  ...
  return (
    <button
      ...
      onClick={() => onOpen(post)}
    >
      <div className="relative mb-3 h-[150px] w-[150px] overflow-hidden rounded-[10px]">
        <Image src={imageSrc} alt={post.title} fill className="object-cover" sizes="150px" />
      ...
      <p className="mb-2 text-sm text-[rgba(255,255,255,0.7)] lg:text-base">{post.claim_summary}</p>
